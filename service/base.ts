 ...
 * 处理fetch api返回的数据对象
 * @params
 * response: fetch返回的结果对象，并且里面的body是一个readStream
 * onData:
 * @extend
 *
 */
...
const baseFetch = (
  ...
  console.log("urlWithPrefix===>>>", urlWithPrefix);
...
export const ssePost = (
  ...
  const contentType = options.headers.get("Content-Type");
  if (!contentType) options.headers.set("Content-Type", ContentType.stream);
  ...
  globalThis
    ...
    .then(async (res: any) => {
      ...
      // res.headers.forEach((it) => console.log(it));
